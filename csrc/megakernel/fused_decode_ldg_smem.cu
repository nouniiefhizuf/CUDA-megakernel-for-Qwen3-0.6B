/**
 * Fused Decode with __ldg() cached reads + Shared Memory Caching
 *
 * Optimization: Cache g_normalized and g_activations in shared memory
 * during matmul phases so all warps read from fast shared memory instead
 * of repeatedly reading from global memory.
 *
 * HIDDEN_SIZE = 1024 floats = 4KB per vector
 * LDG_BLOCK_SIZE = 256 threads, LDG_NUM_WARPS = 8 warps
 */
...
__device__ __forceinline__ float ldg_smem_warp_reduce_sum(float val) {
    #pragma unroll
...
__device__ __forceinline__ float ldg_smem_silu(float x) {
    return x / (1.0f + expf(-x));
}

// =============================================================================
// Optimized matvec with __ldg and SHARED MEMORY CACHING for input vector
// =============================================================================

__device__ void ldg_smem_matvec_qkv(
    cg::grid_group& grid,
    ...
    // Shared memory for caching the normalized input vector
    // All warps in this block will read from this instead of global memory
    ...
    if (block_id == 0) {
        ...
        local_sum_sq = ldg_smem_warp_reduce_sum(local_sum_sq);
        if (lane_id == 0) {
        ...
        if (warp_id == 0) {
            ...
            sum = ldg_smem_warp_reduce_sum(sum);
            if (lane_id == 0) {
    ...
    // ==== OPTIMIZATION V2: Use warp-local register caching ====
    // Each warp loads the portion of g_normalized it will access into registers
    // This avoids shared memory sync overhead while still reducing global reads
    ...
    // Pre-load input vector to registers (each thread loads 4 floats)
    // HIDDEN_SIZE=1024, WARP_SIZE=32, each thread handles 32 elements across iterations
    float reg_input[8];  // 8 vec4 loads = 32 elements per thread
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        int k = lane_id * 4 + i * WARP_SIZE * 4;
        if (k < HIDDEN_SIZE) {
            // Use float4 load for coalesced access
            float4 tmp = *reinterpret_cast<const float4*>(g_normalized + k);
            // Store first element, we'll reconstruct the sum inline
            reg_input[i] = tmp.x + tmp.y + tmp.z + tmp.w;  // Placeholder, real impl below
        }
    }
    
    // Actually, the register approach is complex. Let's try a simpler optimization:
    // Load to shared memory but overlap with computation using warp-level scheduling
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += LDG_BLOCK_SIZE) {
        s_normalized[i] = g_normalized[i];
    }
    __syncthreads();
    ...
    for (int m_base = row_start; m_base < row_end; m_base += LDG_NUM_WARPS) {
        ...
        if (m < row_end) {
            ...
            // Use vec4 loads with __ldg for weights, read input from SHARED MEMORY
            float sum = 0.0f;
            #pragma unroll 8
            for (int k = lane_id * 4; k < HIDDEN_SIZE; k += WARP_SIZE * 4) {
                ...
                // Read from shared memory instead of global g_normalized
                sum += __bfloat162float(w_ptr[0]) * s_normalized[k] +
                       __bfloat162float(w_ptr[1]) * s_normalized[k+1] +
                       __bfloat162float(w_ptr[2]) * s_normalized[k+2] +
                       __bfloat162float(w_ptr[3]) * s_normalized[k+3];
            }
...
// =============================================================================
// QK Norm + RoPE + KV Cache (unchanged from original)
// =============================================================================

__device__ void ldg_smem_qk_norm_rope_cache(
    cg::grid_group& grid,
    ...
    for (int h = q_head_start + warp_id; h < q_head_end; h += LDG_NUM_WARPS) {
        ...
        sum_sq = ldg_smem_warp_reduce_sum(sum_sq);
...
// =============================================================================
// Attention (unchanged from original)
// =============================================================================

__device__ void ldg_smem_prefetch_weights_l2(
...
__device__ void ldg_smem_attention(
    cg::grid_group& grid,
    ...
    if (block_id >= ATTN_BLOCKS) {
        ...
        if (prefetch_block_id < num_prefetch_blocks / 3) {
            ...
            ldg_smem_prefetch_weights_l2(o_weight + start, elems_per_block);
        }
            ...
            ldg_smem_prefetch_weights_l2(gate_weight + start, elems_per_block);
        }
        else {
            int adjusted_id = prefetch_block_id - 2 * num_prefetch_blocks / 3;
            int elems_per_block = (HIDDEN_SIZE * INTERMEDIATE_SIZE) / (num_prefetch_blocks / 3 + 1);
            int start = adjusted_id * elems_per_block;
            ldg_smem_prefetch_weights_l2(up_weight + start, elems_per_block);
        }
    ...
    for (int qh = head_start; qh < head_end; qh++) {
        ...
        for (int pos = warp_id; pos < cache_len; pos += LDG_NUM_WARPS) {
            ...
            score = ldg_smem_warp_reduce_sum(score) * attn_scale;
...
// =============================================================================
// O Projection + Residual + PostNorm + MLP with SHARED MEMORY CACHING
// =============================================================================

__device__ void ldg_smem_o_proj_postnorm_mlp(
    cg::grid_group& grid,
    const __nv_bfloat16* __restrict__ o_weight,
    ...
    // Shared memory for caching the activations vector
    ...
    // ==== OPTIMIZATION: Load g_activations into shared memory once per block ====
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += LDG_BLOCK_SIZE) {
        s_activations[i] = g_activations[i];
    }
    __syncthreads();

    // Gate + Up + SiLU - reading from SHARED MEMORY
    ...
    for (int m_base = int_start; m_base < int_end; m_base += LDG_NUM_WARPS) {
        ...
        if (m < int_end) {
            ...
            for (int k = lane_id * 4; k < HIDDEN_SIZE; k += WARP_SIZE * 4) {
                ...
                // Read from shared memory instead of global g_activations
                gate_sum += __bfloat162float(g_ptr[0]) * s_activations[k] +
                            __bfloat162float(g_ptr[1]) * s_activations[k+1] +
                            __bfloat162float(g_ptr[2]) * s_activations[k+2] +
                            __bfloat162float(g_ptr[3]) * s_activations[k+3];

                up_sum += __bfloat162float(u_ptr[0]) * s_activations[k] +
                          __bfloat162float(u_ptr[1]) * s_activations[k+1] +
                          __bfloat162float(u_ptr[2]) * s_activations[k+2] +
                          __bfloat162float(u_ptr[3]) * s_activations[k+3];
            }

            gate_sum = ldg_smem_warp_reduce_sum(gate_sum);
            up_sum = ldg_smem_warp_reduce_sum(up_sum);

            if (lane_id == 0) {
                g_mlp_intermediate[m] = ldg_smem_silu(gate_sum) * up_sum;
            }
        }
    }
...
ldg_smem_decode_kernel(
    int input_token_id,
    const __nv_bfloat16* __restrict__ embed_weight,
    const LDGSmemLayerWeights* __restrict__ layer_weights,
    ...
    for (int layer = 0; layer < num_layers; layer++) {
        const LDGSmemLayerWeights& w = layer_weights[layer];
...
// =============================================================================
// LM Head (same as original)
// =============================================================================

__global__ void ldg_smem_lm_head_phase1(
...
__global__ void ldg_smem_lm_head_phase2(
...
extern "C" void launch_ldg_smem_decode(
    int input_token_id,
    int* output_token_id,
    const void* embed_weight,
    const LDGSmemLayerWeights* layer_weights,
    ...
    ldg_smem_lm_head_phase1<<<LDG_LM_NUM_BLOCKS, LDG_LM_BLOCK_SIZE, 0, stream>>>(
    ...
    ldg_smem_lm_head_phase2<<<1, 256, 0, stream>>>(

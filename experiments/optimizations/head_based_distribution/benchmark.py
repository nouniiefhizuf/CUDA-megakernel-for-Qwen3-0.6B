#!/usr/bin/env python3
"""
Benchmark and verify Head-Based Distribution optimization.

This optimization assigns blocks to Q heads (82 blocks / 16 heads ≈ 5 blocks per head).
Only leader blocks perform QKV/attention work, eliminating grid.sync between QKV and attention.
...
- Eliminates some grid.sync() calls by having head groups work independently
- Trade-off: Only leader blocks do QKV, others idle during that phase
...
def compile_original_kernel():
    ...
    module = load_inline(
        name="original_kernel_hbd_benchmark",
        cpp_sources=[cpp_src],
...
def compile_optimized_kernel():
    """Compile the head-based distribution optimized kernel."""
    project_root, kernel_dir = get_project_paths()
    ...
    cpp_src = get_cpp_bindings("Optimized", "HBDLayerWeights", "launch_hbd_decode",
                               "init_hbd_layer_weights", "init_hbd_embed_weight", "decode_hbd")

    module = load_inline(
        name="hbd_kernel_benchmark",
        cpp_sources=[cpp_src],
        cuda_sources=[cuda_src],
        extra_cuda_cflags=[
            "-O3",
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Head-Based Distribution optimization")
    ...
    print("="*60)
    print("Head-Based Distribution Optimization Benchmark")
    print("="*60)
    print("\nOptimization: Assign 5 blocks per Q head (82 blocks / 16 heads)")
    print("Leader blocks compute QKV, all blocks in group do attention")
    print("Expected benefit: Reduce cross-block dependencies in QKV phase")
    print()
    ...
    optimized = KernelWrapper(
        optimized_kernel,
        "init_hbd_layer_weights",
        "init_hbd_embed_weight",
        "decode_hbd",
        state_dict
    )
    ...
    # Analysis
    time_diff_us = (avg_orig - avg_opt) * 1000

    print(f"\n{'='*60}")
    print("ANALYSIS")
    print(f"{'='*60}")
    print(f"  Time difference per token: {time_diff_us:.1f} us")
    if time_diff_us > 0:
        print(f"  Optimization effective: {time_diff_us:.1f} us saved per token")
    else:
        print(f"  Optimization not effective: {-time_diff_us:.1f} us overhead per token")
    ...
    print(f"  Improvement: {'+' if tok_per_s_opt > tok_per_s_orig else ''}{tok_per_s_opt - tok_per_s_orig:.1f} tok/s ({(avg_speedup-1)*100:.1f}%)")

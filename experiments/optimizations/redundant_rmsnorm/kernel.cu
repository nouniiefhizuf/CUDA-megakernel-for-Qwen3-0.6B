/**
 * Fused Decode with Redundant RMSNorm Optimization
 *
 * Key optimization: All blocks compute RMSNorm redundantly instead of only block 0.
 * This eliminates grid.sync() calls after RMSNorm phases.
 *
 * Trade-off:
 * - Each of 82 blocks reads 1024 elements (2KB in bfloat16) - fits in L2 cache
 * - Redundant compute: 82 blocks do the same reduction
 * - Benefit: Eliminates 2 grid.sync() per layer = 56 syncs for 28 layers
 *
 * The hidden state (1024 elements = 2KB) fits in L2 cache after first block reads it,
 * so subsequent block reads are L2 hits with ~200 cycle latency.
 */
...
constexpr int RRMSNORM_NUM_BLOCKS = 82;
constexpr int RRMSNORM_BLOCK_SIZE = 256;
constexpr int RRMSNORM_NUM_WARPS = RRMSNORM_BLOCK_SIZE / WARP_SIZE;
constexpr float RRMSNORM_EPS = 1e-6f;

// LM head
constexpr int RRMSNORM_LM_NUM_BLOCKS = 1184;
constexpr int RRMSNORM_LM_BLOCK_SIZE = 256;
constexpr int RRMSNORM_VOCAB_SIZE = 151936;

struct RRMSNormLayerWeights {
...
__device__ __forceinline__ float rrmsnorm_warp_reduce_sum(float val) {
    #pragma unroll
...
__device__ __forceinline__ float rrmsnorm_silu(float x) {
    return x / (1.0f + expf(-x));
}

// =============================================================================
// Redundant RMSNorm Helper - ALL blocks compute the full reduction
// =============================================================================

/**
 * Redundant RMSNorm: Every block computes the same normalization.
 *
 * Each block:
 * 1. Loads all HIDDEN_SIZE elements from input (L2 cached after first block)
 * 2. Computes sum of squares locally
 * 3. Reduces within block to get rstd
 * 4. Writes normalized output
 *
 * All blocks write the same values, eliminating need for grid.sync().
 */
__device__ void redundant_rmsnorm(
    const __nv_bfloat16* __restrict__ input,
    const __nv_bfloat16* __restrict__ weight,
    float* __restrict__ output,
    float* __restrict__ residual_out  // Optional: can be nullptr
) {
    __shared__ float smem[HIDDEN_SIZE];
    __shared__ float smem_reduce[RRMSNORM_NUM_WARPS];

    int warp_id = threadIdx.x / WARP_SIZE;
    int lane_id = threadIdx.x % WARP_SIZE;

    // Phase 1: Load all elements and compute partial sum of squares
    float local_sum_sq = 0.0f;

    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float v = __bfloat162float(__ldg(input + i));
        smem[i] = v;
        if (residual_out != nullptr) {
            residual_out[i] = v;  // All blocks write same value
        }
        local_sum_sq += v * v;
    }

    // Phase 2: Reduce within block
    local_sum_sq = rrmsnorm_warp_reduce_sum(local_sum_sq);
    if (lane_id == 0) {
    ...
    if (warp_id == 0) {
        float sum = (lane_id < RRMSNORM_NUM_WARPS) ? smem_reduce[lane_id] : 0.0f;
        sum = rrmsnorm_warp_reduce_sum(sum);
        if (lane_id == 0) {
            smem_reduce[0] = rsqrtf(sum / float(HIDDEN_SIZE) + RRMSNORM_EPS);
        }
    }
    __syncthreads();
    ...
    // All blocks write the same values - redundant but eliminates sync
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float w = __bfloat162float(__ldg(weight + i));
        output[i] = smem[i] * rstd * w;
    }
}

/**
 * Redundant RMSNorm for float input (used for post-attention)
 */
__device__ void redundant_rmsnorm_float(
...
// =============================================================================
// QKV Matvec with Redundant RMSNorm (NO grid.sync after RMSNorm!)
// =============================================================================

__device__ void rrmsnorm_matvec_qkv(
    cg::grid_group& grid,
    const __nv_bfloat16* __restrict__ input,
    ...
    // =========================================================================
    // OPTIMIZATION: ALL blocks compute RMSNorm redundantly
    // Each block reads all 1024 elements (2KB fits in L2 cache)
    // After first block reads, others get L2 cache hits
    // All blocks write same normalized values - no sync needed!
    // =========================================================================

    // Local shared memory for this block's RMSNorm computation
    __shared__ float smem[HIDDEN_SIZE];
    __shared__ float smem_reduce[RRMSNORM_NUM_WARPS];

    float local_sum_sq = 0.0f;

    // Every block loads all elements and computes sum of squares
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float v = __bfloat162float(__ldg(input + i));
        smem[i] = v;
        g_residual[i] = v;  // All blocks write same value (redundant but fast)
        local_sum_sq += v * v;
    }
    ...
    // Apply normalization - all blocks write same values
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float w = __bfloat162float(__ldg(norm_weight + i));
        g_normalized[i] = smem[i] * rstd * w;
    }

    // NO grid.sync() needed here! All blocks have computed the same normalized values
    // The writes are redundant but identical, so any block can proceed with matvec
    // using its local smem copy

    // QKV projection - each block uses its local smem (already has normalized values)
    ...
    for (int m_base = row_start; m_base < row_end; m_base += RRMSNORM_NUM_WARPS) {
        ...
        if (m < row_end) {
            ...
            // Use local smem for input (already normalized in this block)
            float sum = 0.0f;
            #pragma unroll 8
            for (int k = lane_id * 4; k < HIDDEN_SIZE; k += WARP_SIZE * 4) {
                ...
                // Use normalized values from shared memory
                float n0 = smem[k] * rstd * __bfloat162float(__ldg(norm_weight + k));
                float n1 = smem[k+1] * rstd * __bfloat162float(__ldg(norm_weight + k+1));
                float n2 = smem[k+2] * rstd * __bfloat162float(__ldg(norm_weight + k+2));
                float n3 = smem[k+3] * rstd * __bfloat162float(__ldg(norm_weight + k+3));

                sum += __bfloat162float(w_ptr[0]) * n0 +
                       __bfloat162float(w_ptr[1]) * n1 +
                       __bfloat162float(w_ptr[2]) * n2 +
                       __bfloat162float(w_ptr[3]) * n3;
            }
...
__device__ void rrmsnorm_qk_norm_rope_cache(
    cg::grid_group& grid,
    ...
    for (int h = q_head_start + warp_id; h < q_head_end; h += RRMSNORM_NUM_WARPS) {
        ...
        sum_sq = rrmsnorm_warp_reduce_sum(sum_sq);
        float scale = rsqrtf(sum_sq / float(HEAD_DIM) + RRMSNORM_EPS);
    ...
    for (int h = k_head_start + warp_id; h < k_head_end; h += RRMSNORM_NUM_WARPS) {
...
__device__ void rrmsnorm_attention(
    cg::grid_group& grid,
    ...
    __shared__ float s_max_score[RRMSNORM_NUM_WARPS];
    __shared__ float s_sum_exp[RRMSNORM_NUM_WARPS];
    __shared__ float s_out_acc[RRMSNORM_NUM_WARPS][HEAD_DIM];
    ...
    for (int qh = head_start; qh < head_end; qh++) {
        ...
        for (int pos = warp_id; pos < cache_len; pos += RRMSNORM_NUM_WARPS) {
            ...
            score = rrmsnorm_warp_reduce_sum(score) * attn_scale;
        ...
        if (warp_id == 0) {
            ...
            for (int w = 1; w < RRMSNORM_NUM_WARPS; w++) {
            ...
            for (int w = 0; w < RRMSNORM_NUM_WARPS; w++) {
                if (s_max_score[w] > -INFINITY) {
...
// =============================================================================
// O Projection + PostNorm + MLP with Redundant RMSNorm
// =============================================================================

__device__ void rrmsnorm_o_proj_postnorm_mlp(
    cg::grid_group& grid,
    ...
    for (int m_base = hid_start; m_base < hid_end; m_base += RRMSNORM_NUM_WARPS) {
        int m = m_base + warp_id;
    ...
    // =========================================================================
    // OPTIMIZATION: Redundant Post-attention RMSNorm
    // All blocks compute the same normalization, eliminating grid.sync()
    // =========================================================================
    ...
    // Every block loads all g_activations and computes sum of squares
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float v = g_activations[i];
        smem[i] = v;
        g_residual[i] = v;  // All blocks write same value
        local_sum_sq += v * v;
    }
    ...
    // Write normalized values (all blocks write same)
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_BLOCK_SIZE) {
        float w = __bfloat162float(__ldg(post_norm_weight + i));
        g_activations[i] = smem[i] * rstd * w;
    }

    // NO grid.sync() needed! All blocks have the normalized values in smem

    // Gate + Up + SiLU - use local smem for normalized activations
    ...
    for (int m_base = int_start; m_base < int_end; m_base += RRMSNORM_NUM_WARPS) {
        int m = m_base + warp_id;
        ...
        if (m < int_end) {
            ...
            for (int k = lane_id * 4; k < HIDDEN_SIZE; k += WARP_SIZE * 4) {
                ...
                // Use normalized values from local smem
            ...
            gate_sum = rrmsnorm_warp_reduce_sum(gate_sum);
            up_sum = rrmsnorm_warp_reduce_sum(up_sum);

            if (lane_id == 0) {
                g_mlp_intermediate[m] = rrmsnorm_silu(gate_sum) * up_sum;
            }
        }
    }
...
__global__ void __launch_bounds__(RRMSNORM_BLOCK_SIZE, 1)
rrmsnorm_decode_kernel(
    int input_token_id,
    const __nv_bfloat16* __restrict__ embed_weight,
    const RRMSNormLayerWeights* __restrict__ layer_weights,
    ...
    for (int i = block_id * RRMSNORM_BLOCK_SIZE + threadIdx.x; i < HIDDEN_SIZE; i += num_blocks * RRMSNORM_BLOCK_SIZE) {
    ...
    for (int layer = 0; layer < num_layers; layer++) {
        const RRMSNormLayerWeights& w = layer_weights[layer];
    ...
    // Final RMSNorm - also make redundant for consistency
    {
        __shared__ float smem[HIDDEN_SIZE];
        __shared__ float smem_reduce[RRMSNORM_NUM_WARPS];
        int warp_id = threadIdx.x / WARP_SIZE;
...
// =============================================================================
// LM Head (unchanged from original)
// =============================================================================

__global__ void rrmsnorm_lm_head_phase1(
    ...
    for (int i = threadIdx.x; i < HIDDEN_SIZE; i += RRMSNORM_LM_BLOCK_SIZE) {
        s_hidden[i] = hidden[i];
    }
    __syncthreads();
    ...
    int rows_per_block = (RRMSNORM_VOCAB_SIZE + gridDim.x - 1) / gridDim.x;
    int row_start = blockIdx.x * rows_per_block;
    int row_end = min(row_start + rows_per_block, RRMSNORM_VOCAB_SIZE);

    float local_max = -INFINITY;
    int local_max_idx = -1;

    for (int m = row_start + warp_id; m < row_end; m += RRMSNORM_LM_BLOCK_SIZE / WARP_SIZE) {
    ...
    __shared__ float warp_max[RRMSNORM_LM_BLOCK_SIZE / WARP_SIZE];
    __shared__ int warp_idx[RRMSNORM_LM_BLOCK_SIZE / WARP_SIZE];
    ...
    if (warp_id == 0) {
        float max_val = (lane_id < RRMSNORM_LM_BLOCK_SIZE / WARP_SIZE) ? warp_max[lane_id] : -INFINITY;
        int max_idx = (lane_id < RRMSNORM_LM_BLOCK_SIZE / WARP_SIZE) ? warp_idx[lane_id] : -1;
...
__global__ void rrmsnorm_lm_head_phase2(
...
extern "C" void launch_rrmsnorm_decode(
    int input_token_id,
    int* output_token_id,
    const void* embed_weight,
    const RRMSNormLayerWeights* layer_weights,
    ...
    cudaLaunchCooperativeKernel(
        (void*)rrmsnorm_decode_kernel,
        dim3(RRMSNORM_NUM_BLOCKS),
        dim3(RRMSNORM_BLOCK_SIZE),
        kernel_args,
        0,
        stream
    );

    rrmsnorm_lm_head_phase1<<<RRMSNORM_LM_NUM_BLOCKS, RRMSNORM_LM_BLOCK_SIZE, 0, stream>>>(
    ...
    rrmsnorm_lm_head_phase2<<<1, 256, 0, stream>>>(
        (const float*)block_max_vals,
        (const int*)block_max_idxs,
        output_token_id,
        RRMSNORM_LM_NUM_BLOCKS
    );
}
